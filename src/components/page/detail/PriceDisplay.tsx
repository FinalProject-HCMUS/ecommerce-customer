...
const PriceDisplay: React.FC<PriceDisplayProps> = ({
  ...
  return (
    <div className="flex items-center gap-2">
      <span className={sizes[size].current}>{formatCurrency(currentPrice, 'USD')}</span>
      {discountPercentage != 0 && <span className={sizes[size].original}>{formatCurrency(originalPrice, 'USD')}</span>}
      {discountPercentage != 0 && <span className={sizes[size].discount}>-{discountPercentage}%</span>}
    </div>
  )
}
