...
const VirtualTryOn: React.FC<VirtualTryOnProps> = ({ garment }) => {
  ...
  const processOriginalImage = async (imageUrl: string) => {
    ...
    try {
      ...
      // Step 2: Extract face data
      const extractedFaceData = await extractFaceData(imageUrl, segmentationOutput);
      
      if (extractedFaceData) {
        console.log("Face data extracted successfully");
        setFaceData(extractedFaceData);
        
        // Step 3: Create faceless image
        const facelessImageBase64 = await createFacelessImage(imageUrl, extractedFaceData);
  ...
  const handleToggleFaceRecovery = async () => {
    if (!result || !faceData || !isRecoverable) return;
    ...
    if (!recoveredFaceResult) {
      try {
        ...
        const recoveredImageBase64 = await recoverFace(result, faceData);
    ...
    <div className="bg-white flex flex-col items-center px-4 mt-20 mb-20">
      ...
      {isProcessing && (
        ...
        >
          ...
          {processingStage === 'face-processing' && 'Processing face data...'}
          {processingStage === 'tryon' && 'Generating try-on result...'}
          {processingStage === 'face-recovery' && 'Recovering face...'}
        </motion.div>
      )}
      ...
      <div className="w-full max-w-7xl mb-6">
        ...
        >
          <option value="gemini">Faster - less accurate</option>
          <option value="idm-vton">Slower - more accurate</option>
        </select>
      </div>
      ...
      <div className="w-full max-w-7xl grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        ...
        <ImagePanel
          image={facelessImage}
          placeholderText="Faceless Photo"
