...
export const useOrderDetails = (initialOrderId?: string) => {
  ...
  const [orderDetails, setOrderDetails] = useState<OrderDetailWithProductResponse[]>([]);
  const [loading, setLoading] = useState<boolean>(false);

  const fetchOrderDetails = useCallback(async (id?: string): Promise<CustomResponse<OrderDetailWithProductResponse[]> | undefined> => {
