...
const App: React.FC = () => {
  ...
  const { loading: productColorSizeLoading, fetchProductColorSizes } = useProductColorSize()
  ...
  const [productColorSize, setProductColorSize] = useState<ProductColorSizeResponse[]>([])
    ...
    const fetchProductDetails = async () => {
      if (id) {
        ...
        if (sizeRes && productColorSizeData) {
          ...
          const filteredSizes = sizeRes.content.filter((size) => sizeIds.includes(size.id))
          setSizes(filteredSizes)
        }

        if (colorRes && productColorSizeData) {
          const colorIds = productColorSizeData.map((item) => item.colorId)
          const filteredColors = colorRes.content.filter((color) => colorIds.includes(color.id))
  ...
  if (productLoading || imagesLoading || productColorSizeLoading || colorLoading || sizeLoading) {
    return <Loading />
  }
  ...
  return (
    <div className="max-w-7xl mt-10 px-4 py-8 font-sans">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        ...
        {product && <ProductInfo product={product} colors={colors} productColorSize={productColorSize} sizes={sizes} />}
      </div>
