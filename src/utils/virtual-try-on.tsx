 ...
 */
export async function runSegmentation(imageUrl: string, partsToSegment: string[] | null = null): Promise<Object[]> {
    try {
        ...
        if (partsToSegment && partsToSegment.length > 0) {
            ...
            return output.filter((segment: any) => partsToSegment.includes(segment.label));
        }
 ...
 */
export async function processMasks(segmentationOutput, originalImageUrl, selectedParts = null) {
    ...
    const filteredSegments = selectedParts && selectedParts.length > 0
        ? segmentationOutput.filter(segment => selectedParts.includes(segment.label))
        : segmentationOutput;
    ...
    const compositeCanvas = await createCompositeView(filteredSegments, originalImage, originalImageUrl);
 ...
 */
function createMaskImageData(mask) {
...
function blendFace(targetImageData, sourceImageData, faceMask) {
  ...
  for (let y = 0; y < targetImageData.height; y++) {
    for (let x = 0; x < targetImageData.width; x++) {
      ...
      const maskX = Math.floor(sourceX * (faceMask.width / sourceImageData.width));
      const maskY = Math.floor(sourceY * (faceMask.height / sourceImageData.height));
      const maskIndex = maskY * faceMask.width + maskX;
      const maskValue = faceMask.data[maskIndex];
      ...
      if (maskValue > 128) {
        ...
        targetData[targetIndex] = Math.round(sourceData[sourceIndex] * alpha + targetData[targetIndex] * (1 - alpha));
        targetData[targetIndex + 1] = Math.round(sourceData[sourceIndex + 1] * alpha + targetData[targetIndex + 1] * (1 - alpha));
        targetData[targetIndex + 2] = Math.round(sourceData[sourceIndex + 2] * alpha + targetData[targetIndex + 2] * (1 - alpha));
